  ...
  try {
    ...
    for (const item of feed.items || []) {
      ...
      const insert = await db.pool.query(
        ...
        [link, "timeslive", sourceId, content, contentHash, tenantId]
      );
    ...
    res.status(500).json({ error: "Failed to ingest TimesLIVE (Google News RSS)" });
  }
});
