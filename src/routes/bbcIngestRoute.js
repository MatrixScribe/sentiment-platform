...
router.post('/bbc', async (req, res) => {
  try {
    const { feed = "https://feeds.bbci.co.uk/news/rss.xml" } = req.body;
    const tenantId = req.user.tenant_id;

    const sourceId = await getSourceId("BBC");
  ...
  [item.link, "bbc", sourceId, content, contentHash, tenantId]
    ...
    res.status(500).json({ error: "Failed to ingest BBC feed" });
  }
});
