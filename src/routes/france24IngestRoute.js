// src/routes/france24IngestRoute.js
...
router.post('/france24', async (req, res) => {
  try {
    const { feed = "https://www.france24.com/en/rss" } = req.body;
    const tenantId = req.user.tenant_id;

    const sourceId = await getSourceId("France24");
    ...
    for (const item of feedData.items) {
      ...
      const insert = await db.pool.query(
        ...
        [item.link, "france24", sourceId, content, contentHash, tenantId]
      );
    ...
    console.error("France24 ingestion error:", err);
    res.status(500).json({ error: "Failed to ingest France24 feed" });
  }
});
